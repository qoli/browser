...
pub fn build(b: *std.Build) void {
    ...
    addAppleSysrootIncludes(b, libfipsmodule.root_module, target);
    ...
    addAppleSysrootIncludes(b, libcrypto.root_module, target);
    libcrypto.linkLibC();
    ...
    addAppleSysrootIncludes(b, libssl.root_module, target);
    libssl.linkLibC();
    libssl.linkLibCpp();
    ...
    addAppleSysrootIncludesTranslateC(b, ssl_translate, target);
    ...
    addAppleSysrootIncludes(b, libdecrepit.root_module, target);
    ...
    addAppleSysrootIncludes(b, libpki.root_module, target);
    libpki.linkLibC();
    libpki.linkLibCpp();
...
fn addAppleSysrootIncludes(b: *std.Build, mod: *std.Build.Module, target: std.Build.ResolvedTarget) void {
    if (target.result.os.tag != .ios and target.result.os.tag != .tvos) return;
    if (appleSysrootIncludePath(b)) |include_path| {
        mod.addSystemIncludePath(.{ .cwd_relative = include_path });
    }
}

fn addAppleSysrootIncludesTranslateC(
    b: *std.Build,
...
fn appleSysrootIncludePath(b: *std.Build) ?[]const u8 {
    if (b.sysroot) |sysroot| {
        return b.fmt("{s}/usr/include", .{sysroot});
    }
    const sdkroot = std.process.getEnvVarOwned(b.allocator, "SDKROOT") catch null;
    if (sdkroot) |root| {
        defer b.allocator.free(root);
        return b.fmt("{s}/usr/include", .{root});
    }
    return null;
}
